    ...
    for (let i = start; i <= end; i++) {
      elements.push(
        <button
          ...
          aria-label={`Page ${i}`}
          className={`w-10 h-10 flex items-center justify-center rounded text-sm font-medium transition-colors mx-1 focus-visible:ring-2 focus-visible:ring-blue-500 cursor-pointer ${currentPage === i
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div
        className={
          viewMode === "grid"
            ? "grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6 w-full"
            : "flex flex-col space-y-4 w-full"
        }
      >
        {products.map(p => (
          <div key={p.id} className={viewMode === "grid" ? "h-full" : "w-full lg:w-3/4 max-w-2xl mx-auto"}>
